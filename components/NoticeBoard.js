...
export default function Noticeboard(props) {
    return (
      <View style={[styles.sectionContainer, {borderBottomColor: props.color, borderTopColor: props.color}]}>
        <Text style={{fontSize: 22, textAlign: "justify", marginBottom: 10}}>Invite Friend & Get Commission from every ride of them.</Text>
        <Text style={{fontSize: 22, textAlign: "justify", fontWeight: 'bold'}}>Safety First, Safe JourneySave Money</Text>
        <Image style={{ height: 300, width: SCREEN_WIDTH, resizeMode: 'contain' }} source={require('../assets/welcome-image.png')} />
      </View>
    )
}
