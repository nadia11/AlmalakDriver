...
import { View, Text, StyleSheet, Alert, TouchableOpacity, Image, SafeAreaView, ScrollView, Dimensions, ToastAndroid, Animated, Linking } from 'react-native';
...
import TripSummery from './MenuScreens/TripSummery';
import manageDocuments from './MenuScreens/manageDocuments';
...
import changePassword from './MenuScreens/changePassword';
...
export default function MenuStackScreen() {
  return (
    <MenuStack.Navigator screenOptions={Options.APP_OPTIONS.SCREEN_OPTIONS}>
      ...
      <MenuStack.Screen name="changePassword" component={changePassword} options={{ title: "Change Password", headerRight: () => Options.APP_OPTIONS.HEADER_LOGO }} />
...
export const MenuScreen = (props) => {
  ...
  const getUserData = async () => {
    try {
      ...
      const userImageStorage = await AsyncStorage.getItem('userImage')
      if(userImageStorage !== null) { setUserImage( userImageStorage ); }
    } 
  ...
  const showBalanceHandle = () => {
    axios.get(BASE_URL+'/get-wallet-balance/' + mobile)
    .then((response) => {
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <ScrollView style={styles.scrollView}>
        ...
        {/* <TouchableOpacity onPress={() => navigation.navigate('TripSummery') } style={styles.listItem}>
          <Ionicons name="md-stats-chart" size={20} color={Colors.ICON_COLOR} style={styles.leftIcon} />
          <Text style={styles.listItemText}>Trip Summery</Text>
          <Feather name="chevron-right" size={15} color={Colors.TEXT_PRIMARY} style={styles.rightIcon} />
        </TouchableOpacity> */}

        {/* <TouchableOpacity onPress={() => navigation.navigate('manageDocuments') } style={styles.listItem}>
          <Feather name="settings" size={20} color={Colors.ICON_COLOR} style={styles.leftIcon} />
          <Text style={styles.listItemText}>Manage Documents</Text>
          <Feather name="chevron-right" size={15} color={Colors.TEXT_PRIMARY} style={styles.rightIcon} />
        </TouchableOpacity> */}
        ...
        <TouchableOpacity onPress={() => navigation.navigate('editProfile', {name: "Language Preference", mobile: mobile, userName: userName}) } style={styles.listItem}>
          <Ionicons name="create-outline" size={25} color={Colors.ICON_COLOR} style={styles.leftIcon} />
          <Text style={styles.listItemText}>Edit My Profile</Text>
          <Feather name="chevron-right" size={15} color={Colors.TEXT_PRIMARY} style={styles.rightIcon} />
        </TouchableOpacity>

        <TouchableOpacity onPress={() => navigation.navigate('changePassword', {email: email}) } style={styles.listItem}>
          <Feather name="lock" size={25} color={Colors.ICON_COLOR} style={styles.leftIcon} />
        ...
        <TouchableOpacity onPress={() => navigation.navigate('agentLocator')} style={styles.listItem}>
          <MaterialCommunityIcons name="map-marker-radius" size={25} color={Colors.ICON_COLOR} style={styles.leftIcon} />
          <Text style={styles.listItemText}>Agent Locator</Text>
          <Feather name="chevron-right" size={15} color={Colors.TEXT_PRIMARY} style={styles.rightIcon} />
        </TouchableOpacity>
        ...
        {/*  <Text style={styles.listItemText}> Page</Text>*/}
        {/*  <Feather name="chevron-right" size={15} color={Colors.TEXT_PRIMARY} style={styles.rightIcon} />*/}
        {/*</TouchableOpacity>*/}

        <TouchableOpacity onPress={() => Linking.openURL('https://udertaxi.com')} style={styles.listItem}>
