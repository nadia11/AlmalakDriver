...
getUserData = async () => {
  ...
  makeRemoteRequest = async () => {
    ...
    setTimeout(() => {
      axios.get(`${BASE_URL}/driver-trip-list/${this.state.mobile}${this.state.hideCanceledRide ? '/hideCanceledRide' : ""}`)
      .then((response) => {
      ...
      .catch((error) => {
	    console.log("driver-trip-list Error: "+error.message);
  ...
  renderItem = ({ item, index, separators }) => (
    <View style={styles.tripsItem} key={item.trip_id}>
      ...
      {(item.trip_map_screenshot == null && item.end_lat !== null) && (
        <MapView liteMode key={`map_${item.trip_id}`} style={{height: 200}} region={{latitude: item.origin_lat, longitude: item.origin_long, latitudeDelta: 0.015, longitudeDelta: 0.0121}}>
