  ...
  renderItem = ({ item, index, separators }) => (
    <View>
      <ListItem
        ...
        subtitle={item.phoneNumbers.length < 2 ? (item.phoneNumbers.map(phone => phone.label.toUpperCase())+": "+item.phoneNumbers.map(phone => phone.number.replace('+88', "").replace(/ /g, ""))) : ( item.phoneNumbers[0].label.toUpperCase()+": "+item.phoneNumbers[0].number.replace('+88', "").replace(/ /g, ""))} 
        subtitleStyle={{ color: '#888' }}
        leftAvatar={{ backgroundColor: colors[index % colors.length], rounded: true, source: item.hasThumbnail && { uri: item.thumbnailPath }, title: item.displayName[0] }}
