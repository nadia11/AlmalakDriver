  ...
  renderItem = ({ item, index, separators }) => (
    <View>
      <ListItem
        ...
        leftAvatar={{ rounded: true, source: item.picture.thumbnail && { uri: item.picture.thumbnail }, title: item.name[0] }}
        containerStyle={{ borderBottomWidth: 0 }}
        bottomDivider 
        onPress={() => this.handelCheckBox(item.email) }
        // chevron chevron={{ color: '#888' }}
        />
        <CheckBox value={this.state.check[item.email]} key={item.email} onValueChange={() => this.handelCheckBox(item.email) } style={styles.checkbox} />
      </View>
  )
