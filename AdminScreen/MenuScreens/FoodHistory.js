...
getUserData = async () => {
  ...
  makeRemoteRequest = async () => {
    ...
    setTimeout(() => {
      axios.get(`${BASE_URL}/driver-food-delivery-list/${this.state.mobile}${this.state.hideCanceledRide ? '/hideCanceledRide' : ""}`)
      .then((response) => {
      ...
      <SafeAreaView style={styles.container}>
        {!this.state.serverData.length && (
          <View style={{ flex: 1, justifyContent: 'center', alignItems: 'center' }}>
			      <NoRecordIcon title="Food Delivery History" reloadFunction={() => this.makeRemoteRequest()} spinner={this.state.loading} />
          </View>
        )}
